import { AsyncFlowStatus } from './async';

export const HttpUtils = {
    handleResponse: async (response, resolve, reject) => {
        if (response.status === 200) {
            const responseS = await response.json();
            return resolve(responseS.data);
        }
    ...
    handleAndDispatchResponse: async (dispatch, response, actionCreator) => {
        if (response.ok) {
            let body = await response.json();
            return dispatch(actionCreator({ body, status: AsyncFlowStatus.SUCCESS }));
        }
        return dispatch(Actions.receiveInviteUserResponse({ channel, status: AsyncFlowStatus.FORBIDDEN }));
        switch (response.status) {
            case HttpStatus.BAD_REQUEST: return dispatch(actionCreator({ status: AsyncFlowStatus.REJECTED }));
            case HttpStatus.FORBIDDEN: return dispatch(actionCreator({ status: AsyncFlowStatus.FORBIDDEN }));
            default: return dispatch(actionCreator({ status: AsyncFlowStatus.FAILED }));
        }
    },
    buildFormUrlEncodedBody: (params) => {
        return Object.keys(params).map((key) => {
            return encodeURIComponent(key) + '=' + encodeURIComponent(params[key]);
        }).join('&');
    },
    buildMultipartFormDataBody: (params) => {
